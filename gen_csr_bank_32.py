...
# CSR length table
#dict_reg_len={"XLEN":32, "SXLEN":32, "MXLEN":32, "DXLEN":32, "XLEN*TRIGGERS":128, "clog2(VLEN)":7}
dict_reg_len["XLEN"] = 32
dict_reg_len["SXLEN"] = 32
dict_reg_len["MXLEN"] = 32
dict_reg_len["DXLEN"] = 32
dict_reg_len["XLEN*TRIGGERS"] = 128
...
"""
        ...
        with open(output_file_struct, mode='w', newline = '') as codal_struct:
          ...
          for row in reader:
            ...
            rowx_reset = row[header.index('Reset Impl L')]
            ...
            if row[header.index('L71')] not in ['', '-'] and row[header.index('Shadow of')] in ['', '-']:
